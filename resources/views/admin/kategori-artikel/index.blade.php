...
<div class="pg-hd">
  <div>
    ...
    <p>Kelola kategori untuk pengelompokan artikel edukasi.</p>
  </div>
  <div class="pg-hd-acts">
    <a href="{{ route('admin.artikel.index') }}" class="btn btn-outline">
...
<div style="display:grid;grid-template-columns:repeat(3,1fr);gap:16px;margin-bottom:24px">
  <div class="kpi">
    <div class="kpi-icon" style="background:var(--cpl)">
      <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="var(--cp)" stroke-width="2"><path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8z"/><polyline points="14 2 14 8 20 8"/></svg>
    </div>
  ...
  <div class="kpi">
    <div class="kpi-icon" style="background:rgba(59,130,246,.1)">
      <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#3b82f6" stroke-width="2"><path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="16" y1="13" x2="8" y2="13"/><line x1="16" y1="17" x2="8" y2="17"/></svg>
    </div>
    <div><div class="kpi-val">{{ $kategoris->sum('articles_count') }}</div><div class="kpi-label">Total Artikel</div></div>
  </div>
</div>
...
<div class="card">
  ...
  <div class="card-body" style="padding:0">
    ...
    <div style="padding:48px;text-align:center;color:var(--cmt)">
      <svg xmlns="http://www.w3.org/2000/svg" width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" style="margin-bottom:12px;opacity:.4"><path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8z"/><polyline points="14 2 14 8 20 8"/></svg>
      ...
      <table>
        <thead>
          <tr>
            ...
            @endif
            @endforeach
            <th style="width:100px;text-align:center">Jml Artikel</th>
        ...
        <tbody id="kat-tbody">
          ...
          <tr id="row-{{ $kat->id }}">
            ...
            @endif
            @endforeach
            <td style="text-align:center"><span class="badge" style="background:rgba(59,130,246,.1);color:#3b82f6">{{ $kat->articles_count }} artikel</span></td>
            ...
            <td style="text-align:right">
              <div style="display:flex;gap:6px;justify-content:flex-end">
                ...
                <button class="act-btn del" title="Hapus" onclick="deleteKat({{ $kat->id }}, '{{ addslashes($kat->nama) }}', {{ $kat->articles_count }})">
...
<div class="modal-overlay" id="m-add" onclick="handleOC(event,'m-add')">
  <div class="modal">
    <div class="modal-hd">
      <div class="modal-icon"><svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8z"/><polyline points="14 2 14 8 20 8"/></svg></div>
      <div><h3>Tambah Kategori</h3><p>Buat kategori baru untuk artikel.</p></div>
      ...
      <div class="modal-body">
        ...
        <div class="fg">
          ...
          <div id="add-kat-pane-{{ $lang->code }}" class="lang-pane {{ $loop->first ? 'active' : '' }}">
            @if($lang->code === 'id')
            <input type="text" id="add-nama" class="fc" placeholder="Tanaman Herbal" {{ $loop->first ? 'required' : '' }} maxlength="100"
    ...
    <form onsubmit="submitEdit(event)">
      ...
      <div class="modal-body">
        ...
        <div id="edit-rename-warn" style="display:none;background:rgba(245,158,11,.08);border:1px solid rgba(245,158,11,.3);border-radius:var(--r1);padding:10px 12px;font-size:12.5px;color:#b45309">
          ⚠️ Mengubah nama kategori akan otomatis memperbarui semua artikel yang menggunakan kategori ini.
        </div>
      </div>
      <div class="modal-ft">
...
async function submitAdd(e) {
  ...
  const r = await apiFetch("{{ route('admin.kategori-artikel.store') }}", 'POST', payload);
  clearLoading();
...
async function submitEdit(e) {
  ...
  const r = await apiFetch(`/admin/kategori-artikel/${id}`, 'PUT', payload);
  clearLoading();
...
async function toggleKat(id, checkbox) {
  const r = await apiFetch(`/admin/kategori-artikel/${id}/toggle`, 'PATCH');
...
async function deleteKat(id, nama, count) {
  if (count > 0) { showToast(`Tidak bisa dihapus — ada ${count} artikel di kategori ini.`, 'error'); return; }
  if (!confirm(`Hapus kategori "${nama}"?`)) return;
  setLoading('Menghapus...');
  const r = await apiFetch(`/admin/kategori-artikel/${id}`, 'DELETE');
  clearLoading();
